        ...
        <>
          {(showAddBlog || blogEdit) ?
            ...
            <SBox1>
              <Box>
                <SButton1
                  onClick={() => { setShowAddBlog(true); setOpen(true) }}
                >
              ...
              <Box>
                <STypography1 variant='h6' sx={{ color: '#412525' }}>
          ...
          <Grid container>
            {blogs.filter(blog => blog.user === userInfo.id).map(Blog => {
              return (
                <Grid item lg={4} md={6} sm={12} xs={12} key={Blog.uid}>
                  <SBox2 boxShadow={6}>
                    <Box textAlign='right'>
                      {Blog.user === userInfo.id ?
                        <Box sx={{ height: '50px' }}>
                          ...
                          <EditIcon
                            fontSize='small'
                            sx={{ color: '#8A2244', p: 1 }}
                            onClick={() => { GetBlogID(Blog.uid, Blog.title, Blog.article); setShowAddBlog(false); setOpenEdit(true) }}
                          />
                        </Box> :
                        <Box sx={{ height: '50px' }} />
                      }
